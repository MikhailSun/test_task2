...
lambda_tube=1 #теплопроводность материала трубы, Вт/м/К (можно сделать как функцию)
...
res_t_oil_in_tube={77:np.nan,
                   ...
                   27: np.nan,
                   30: np.nan,
                   32: np.nan} #температура масла внутри трубы вдоль трубы в заданный момент времени
res_t_water_in_tube={77:np.nan,
                     20:np.nan,
                     27:np.nan,
                     30:np.nan,
                     32:np.nan} #температура воды внутри трубы вдоль трубы в заданный момент времени
res_t_tube={77:np.nan, #распределенеи температуры в трубе
             20:np.nan,
             27:np.nan,
             30:np.nan,
             32:np.nan}
...
for i in range(1,int(T/dt)):
    ...
    for t_ in res_t_tube.keys():
        if (abs(t_ - t) < dt * 0.5):
            dist=[i*L_element for i in range(N)]
            temp_of_tube=[sum(temps)/len(temps) for temps in t_tube]
            res_t_tube[t_] = [dist,temp_of_tube]
...
for (time1,dist_temp_oil), (time2,dist_temp_water), (time3,dist_temp_tube) in zip(res_t_oil_in_tube.items(), res_t_water_in_tube.items(), res_t_tube.items()):
    mc.Chart(points_for_plot=[{'x': dist_temp_oil[0], 'y':dist_temp_oil[1], 'label': f'Oil: time={time1}','c':'red'},
                              {'x': dist_temp_water[0], 'y':dist_temp_water[1], 'label': f'Water: time={time2}','c':'blue'},
                              {'x': dist_temp_tube[0], 'y':dist_temp_tube[1], 'label': f'Tube: time={time3}','c':'black'},],
